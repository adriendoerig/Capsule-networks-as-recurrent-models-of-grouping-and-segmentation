...
def plot_uncrowding_results(results, categories, n_idx, save=None):
    ...
    if n_idx==2:
        ...
        # n_idx can currently only be 2 or 3
        raise SystemExit('\nPROBLEM: n_idx can currently only be =2 or =3! '
...
################################
def routing_by_agreement(caps2_predicted, batch_size, iter_routing, parameters):
    '''
    ...
    iter_routing: tensor
                  Contains the number of routing iterations
    parameters: flags
    ...
    def routing_body(raw_weights, caps2_output, counter):
        ...
        # it here but we need to have the same inputs/outputs for the routing body.
...
def secondary_caps_layer(caps1_output, batch_size, iter_routing, parameters,
                         W_init=None):
    '''
...
def compute_reconstruction(decoder_input,  parameters, phase=True,
    ...
    with tf.name_scope('decoder_reconstruction'):
        ...
        # Alternatively: this version is based on deconvolution
            ...
            # Flatten the output to make it equal to the output of the
