...
class stim_maker_fn:
    ...
    def __init__(self, imSize, shapeSize, barWidth, offset=1):
        ...
        # Fix the numer of shapes in the input image (except lines):
        self.shape_repetitions = 2
        # Shapes must face one another (relevant for cuboids):
        self.face_each_other = 1
        # Lines can be repeated more often, to make sure that the network does not
        # learn too basic differences between the shape types:
        self.line_repetitions = [2, 4, 6, 8]
        # Define the maximal offset between the stimuli
    ...
    def plotAllStim(self, shape_types, offset):
        ...
        # Due to varying widths, we need to get the total width
    ...
    def makeTrainBatch(self, shape_types, batch_size, reduce_df=False):
        '''
        ...
        # Define the max random offset between the stimuli:
    ...
    def makeTestBatch(self, crowding_config, batch_size, stim_idx=None, reduce_df=False):
        '''
        ...
        for idx_batch in range(batch_size):
            ...
            if idx == 0:
                # Vernier-alone condition:
                totalWidth = vernierpatch_width

                if reduce_df:
                    # We want to make the degrees of freedom for position on the
                    # x axis fair.
                ...
                # Flankers plus vernier condition
                totalWidth = 0
